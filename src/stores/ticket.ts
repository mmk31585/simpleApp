...
export const useTicketStore = defineStore('ticket', () => {
  ...
  const ticketCounts = computed(() => {
    const counts = { open: 0, 'in-progress': 0, closed: 0, total: tickets.value.length }
  ...
  // ⬇️ محاسبه‌ی امنِ زمانِ سپری‌شده (زنده)
  const getCurrentTimeSpentMinutes = (ticketId: string): number => {
    const t = findTicket(ticketId)
    if (!t) return 0
    const base = Math.max(0, Math.floor(t.timeSpent || 0))
    if (t.isTimerActive && t.timerStartTime) {
      const startMs = Date.parse(t.timerStartTime)
      if (Number.isFinite(startMs)) {
        const extra = Math.floor((nowMs.value - startMs) / 60000)
