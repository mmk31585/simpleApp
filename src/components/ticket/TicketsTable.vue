  ...
  <div class="tw:bg-white tw:dark:bg-[#1a1a1a] tw:rounded-xl tw:shadow-md tw:overflow-hidden">
    ...
    <div class="tw:overflow-x-auto">
      <a-table
        ...
        :scroll="{ x: 1100 }"
        tableLayout="fixed"
        :customRow="(record) => ({ onClick: () => onRowClick(record) })"
...
const pagination = {
  ...
  showTotal: (total: number, range: [number, number]) => `${range[0]}-${range[1]} از ${total} تیکت`,
...
const handleBulkStatusUpdate = (status: string) => {
  ticketStore.bulkUpdateStatus(selectedRowKeys.value, status as any);
  message.success(`وضعیت ${selectedRowKeys.value.length} تیکت به‌روزرسانی شد`);
};
